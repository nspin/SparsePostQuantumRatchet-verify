...
"""),
    ...
    # https://github.com/AeneasVerif/aeneas/issues/1043
    mk_regex(r'next := core\.iter\.adapters\.map\.Map\.Insts\.CoreIterTraitsIteratorIterator\.next[\s\S]*?(?=\n  step_by)',
    ...
    mk_regex(r'impl_def(\s+[\w.]+\.Insts\.ProstMessageMessage\s*:[\s\S]*?):= \{[\s\S]*?\n\}',
    ...
    mk_find("""let chain ←
              ...
              """let chain' /- See https://github.com/Beneficial-AI-Foundation/SparsePostQuantumRatchet-verify/issues/101 -/ ←
                match val2.key with"""),
    mk_find('chain.Chain.send_key chain i3',
              "chain.Chain.send_key chain' i3 /- #101 rename -/"),
    mk_find('chain.Chain.send_key chain i1',
    ...
    mk_find("""let chain ←
              ...
              """let chain' /- See https://github.com/Beneficial-AI-Foundation/SparsePostQuantumRatchet-verify/issues/101 -/ ←
                    match val3.key with"""),
    mk_find('chain.Chain.into_pb chain\n',
              "chain.Chain.into_pb chain' /- #101 rename -/\n"),
    mk_find('chain.Chain.recv_key chain msg_key_epoch index',
    ...
    # https://github.com/Beneficial-AI-Foundation/SparsePostQuantumRatchet-verify/issues/101
    mk_find("""let v1 := read_discriminant v
    ...
    # https://github.com/AeneasVerif/aeneas/issues/1046
    mk_regex(r'(def proto\.pq_ratchet\.(?:pq_ratchet_state\.Inner|v1_msg\.InnerMsg|v1_state\.InnerState)\.merge\n[\s\S]*?):= do[\s\S]*?(?=\n/-- )',
